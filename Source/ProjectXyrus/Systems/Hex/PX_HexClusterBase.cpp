...
void APX_HexClusterBase::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(APX_HexClusterBase, ClusterSettings);
	DOREPLIFETIME(APX_HexClusterBase, ClusterCells);
}

APX_HexClusterBase::APX_HexClusterBase()
{
...
void APX_HexClusterBase::PostInitializeComponents()
{
...
void APX_HexClusterBase::PostEditChangeChainProperty(FPropertyChangedChainEvent& PropertyChangedEvent)
{
	if (GenerateOfFly)
	{
...
bool APX_HexClusterBase::SetupCluster(const FPX_HexClusterSettings& NewSettings)
{
	ClusterSettings = NewSettings;
	return true;
}

UPX_HexBaseComponent* APX_HexClusterBase::GetHexagon_ByCubeCoordinate(const FPX_HexCubeCoordinate& HexCoordinate)
{
	return nullptr;
}

UPX_HexBaseComponent* APX_HexClusterBase::GetHexagon_ByGlobalCoordinate(const FVector& GlobalCoordinate)
{
	return nullptr;
}

bool APX_HexClusterBase::GenerateClusterFromSettings()
{
	ClearCluster();
...
bool APX_HexClusterBase::GenerateCells()
{
	...
	for (int32 CellIndex_X = 0; CellIndex_X < ClusterSettings.ClusterCells_X; CellIndex_X++)
	{
		...
		{
			FPX_HexOffsetCoordinate FirstHexagonOffsetForCell;
			...
			// create cluster
			APX_HexClusterCellBase* ClusterCell = GetWorld()->SpawnActor<APX_HexClusterCellBase>(
...
bool APX_HexClusterBase::GenerateHexagonsInCell(APX_HexClusterCellBase* ClusterCell, const FVector& ClusterLocation,
	const FPX_HexOffsetCoordinate& FirstHexagonOffset, const FVector2d& HexParameters, const bool& HexagonStartFromTop)
{
	...
	for (int32 HexIndex_X = 0; HexIndex_X < ClusterSettings.HexagonClusterCellSize; HexIndex_X++)
	{
		...
		{
			FPX_HexOffsetCoordinate OddqCoordinate;
			...
			if (UPX_HexBaseComponent* Hexagon = NewObject<UPX_HexBaseComponent>(ClusterCell, ClusterSettings.HexagonClass))
			{
...
bool APX_HexClusterBase::ShouldGenerateHexagon(FPX_HexOffsetCoordinate& HexOffsetCoordinate)
{
	return true;
}

void APX_HexClusterBase::ClearCluster()
{
